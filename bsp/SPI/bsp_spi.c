...
SPI_instance_t *SPI_Register(spi_init_config_t *conf)
{
	...
	instance->id            = conf->id;
	instance->cs_flag       = conf->cs_flag;
	//    if (instance->spi_handle->Instance == SPI1)
	//    {
	//        instance->cs_pin_state = &SPI_Device_On_Going[0];
	//    }
	//    else if (instance->spi_handle->Instance == SPI2)
	//    {
	//        instance->cs_pin_state = &SPI_Device_On_Going[1];
	//    }
	//    else
	//    {
	//        while (1)
...
void SPI_Transmit_Receive(SPI_instance_t *spi_ins, uint8_t *ptr_data_rx, uint8_t *ptr_data_tx, uint8_t len)
{
	...
	// {
	// 	while( !SPI_Device_On_Going[0] )
	// 	{
	// 	};
	// }
	// else if( spi_ins->spi_handle->Instance == SPI2 )
	// {
	// 	while( !SPI_Device_On_Going[1] )
	// 	{
	// 	};
	// }
	// 拉低片选,开始传输
...
void HAL_SPI_RxCpltCallback(SPI_HandleTypeDef *hspi)
{
	for (size_t i = 0 ; i < idx ; i++)
	{
		    ...
		    HAL_GPIO_ReadPin(spi_instances[i]->GPIOx, spi_instances[i]->cs_pin) == GPIO_PIN_RESET)
		{
			...
			{ // 回调函数不为空, 则调用回调函数
				spi_instances[i]->callback(spi_instances[i]);
			}
			return;
		}
	}
