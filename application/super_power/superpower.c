...
Chassis_Power_instance_t *chassis_power;

void Super_Power_Init(void)
{
	chassis_power = Chassis_Power_Init();
	Chassis_Power_send_cmd(120, 60);
}
...
static void Chassis_Power_Decode(CAN_instance_t *can_instance)
{
    ...
    capEnergy = data[7];
    chassis_power->ChassisPower = chassisPower;
    chassis_power->ChassisPowerLimit   = chassisPowerLimit;
    chassis_power->CapEnergy = capEnergy;
}
...
void Chassis_Power_send_cmd(uint16_t power_limit, uint16_t RefereeEnergyBuffer)
{
    ...
    // tx_buff[0]：使能DCDC和系统重启位默认0，保留位0
    tx_buff[0] = 0x00;
    ...
    tx_buff[3] = RefereeEnergyBuffer & 0xFF;
    tx_buff[4] = (RefereeEnergyBuffer >> 8) & 0xFF;
    ...
    memcpy(chassis_power->Chassis_power_can_instance->tx_buff, tx_buff, sizeof(tx_buff));
    CAN_Transmit(chassis_power->Chassis_power_can_instance, 1);
}