...
uint8_t seq = 0;
int ui_self_id = 103;//此处依据机器人id自行更改
...
void autonomous_sentinel_decision(sentry_cmd_t *msg){//
    ...
    msg->header.crc8 = calc_crc8((uint8_t *) msg, 4);
    msg->header.cmd_id = 0x0301;
    msg->header.sub_id = 0x0120;
    //哨兵补充结构体对所需指令设置--uint32_t sentry_cmd
    msg->crc16 = calc_crc16((uint8_t *) msg, 11);
}

/* 8字节指令如下：
uint8_t radar_cmd：开局为0，修改此值即可请求触发双倍易伤，若此时雷达拥有触发双倍易伤的机会，则可触发。此值的变化需要单调递增且每次仅能增加1
以下每个字节均为ASCII码编码的字母或数字
uint8_t password_cmd ：当值为1时，修改此值即可更新己方加密密钥；当值为2时，修改此值即可将雷达破解的对方密钥传输给服务器以验证是否正确
接下来的6字节数据为密钥值，用于破解敌方密钥*/
void autonomous_radar_decision(radar_cmd_t *msg){
    ...
    msg->header.crc8 = calc_crc8((uint8_t *) msg, 4);
    msg->header.cmd_id = 0x0301;
    msg->header.sub_id = 0x0121;
    //雷达需补充结构体对所需指令设置
    msg->crc16 = calc_crc16((uint8_t *) msg, 15);
}

sentry_cmd_t sentinel_decision;
radar_cmd_t radar_decision;
void Auto_sentrycmd_set(){
    autonomous_sentinel_decision(&sentinel_decision);
    SEND_MESSAGE((uint8_t *) &sentinel_decision, sizeof(sentinel_decision));
}

void Auto_radarcmd_set(){
    autonomous_radar_decision(&radar_decision);
