...
//void update_shooter_ui(float gimbal_delta, uint8_t cap_level, uint8_t auto_status, uint8_t fric_status, uint8_t status_num) // 更新所有UI的函数，gimbal_delta为（云台角度-底盘角度），正中间为0度，顺时针为正，弧度制；cap_level为超电剩余百分比；auto_status为自瞄状态，2为自瞄中，1为自瞄已准备好，否则为0；fric_status为摩擦轮状态，开启为1，否则为0
//{
...
//	ui_Infantry_group2_error_num->number = status_num;
...
//		
//		supercap_energy = (SuperCap_Data->CapEnergy / 255.0f * 100.0f);
